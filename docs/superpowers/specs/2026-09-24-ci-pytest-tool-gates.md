...
- The ~3.5k-line pytest suite in `tests/` (measured; CHANGELOG:L17 says "452 passed, 5 skipped" at vendoring time), including the in-tree security regression suite (symlink and ownership locks, Unicode sanitization, injection scan, DOCX safety), cannot fail a pull request.
...
- The extraction and pack pipeline is never compile-checked, so an import break ships green.
...
2. The job installs the project with the extras the suite needs (documented runbook uses `.[all]`; the plan may substitute a narrower named-extra set if it verifies the suite still passes and says so in the plan), runs `pytest`, runs the five-tool self-check loop, and runs `python3 -m py_compile tools/*.py scripts/extract.py` as a smoke.
3. CONTRIBUTING.md documents one runbook command sequence that mirrors the CI tests job, so a contributor can reproduce the gate locally.
...
- Steps: checkout; `actions/setup-python` pinned to a version consistent with `pyproject.toml`'s `requires-python`, with `cache: pip`; `pip install -e ".[all]"` (or the plan's verified narrower set); `pytest`; the self-check loop from CONTRIBUTING; the `py_compile` smoke.
...
1. Existing workflow steps stay untouched; only additive job and additive CONTRIBUTING block.
...
4. Windows dev box, Linux runners: the runbook block notes any bash-specific syntax; the plan verifies the suite passes locally on Windows before relying on it as the gate.
